...
Shader "socialPointCG/sCG_Fx_glowSmoke_SCREEN" {
	Properties{
		_Color("Tint", Color) = (1,1,1,1)
		_AlphaScale("Alpha Scale", float) = 1
		_DF("Clouds map", 2D) = "white" {}
		_MK("Mask map", 2D) = "white" {}
		_UVscale("Cloud: Scale(xy) AND Speed (zw)", Vector) = (1,1,0,0)
	}
		...
		struct vertexOutput {
			...
			half2 uv0 : TEXCOORD0;
			fixed4 VC : COLOR;
			float2 uv0Mov1 : TEXCOORD1;
			float2 uv0Mov2 : TEXCOORD2;
		};
		...
		vertexOutput vert(vertexInput v)
		{
			...
			o.uv0Mov1 = v.uv0.xy * _UVscale.xy + (t * _UVscale.ba);
			o.uv0Mov2 = v.uv0.xy * (_UVscale.xy * 0.75) + (t * 0.75 * _UVscale.ab);
		...
		{
			fixed4 DF1 = tex2D(_DF, i.uv0Mov1);
			fixed4 DF2 = tex2D(_DF, i.uv0Mov2);

			fixed4 DF = DF1 + DF2 * 0.5;

			fixed4 MK = tex2D(_MK, i.uv0);

			//fixed4 Complete = pow(DF * MK, _AlphaScale) * _Color;
			fixed4 Complete = DF * MK * _Color * i.VC.r * _AlphaScale * i.VC * i.VC.a;
